...
设置页底部的“关于”面板提供当前版本、项目仓库、发布页、问题反馈和许可证链接。
...
在“总结设置”中可调整“每篇笔记最多输出的 token 数”，默认 **3000**，可设为 **1000–16000**。这是每次总结请求的输出上限，不是整批任务的总额度；具体模型可能有更低的限制。

插件在首次请求中就会告诉模型输出上限，并要求把正文控制在约上限的 75% 以内，按所选章节平均规划篇幅、优先保留关键信息。`max_tokens` 是 API 的硬上限；提示词只能引导模型，不能保证它精确计算 token 或一定完整输出。如果 API 返回 `length`，或结果缺少所选章节，插件会保存已生成的内容，并在**笔记底部**提示总结不完整。插件不会自动重试；可减少所选章节或提高输出上限后手动重新生成。

## 费用参考

以下数字是按常见论文长度和默认设置得到的粗略估算，并非固定价格；实际消耗会随 PDF 文本长度、所选笔记章节、模型版本及平台计费规则变化。

- **DeepSeek**：高峰时段约可生成 **12 篇笔记/元**，闲时约可生成 **24 篇笔记/元**。
- **Qwen（阿里云百炼）**：有多种提供 **100 万（1M）免费 token 额度**的模型可选；100 万 token 大约可以生成 **40 篇笔记**。

## 使用方法

1. 在 Zotero 中选中一篇或多篇带 PDF 附件的文献。
2. 右键选择“使用 AI 总结 PDF 并添加笔记”。
3. 等待右下角进度提示完成；每篇返回后会显示该篇及本次任务累计的输入、输出 token。
4. 展开文献条目，在其子笔记中查看“AI 文献总结”。

如果直接选中一个 PDF 附件，插件会将笔记添加到该 PDF 的父文献条目下。没有 PDF 的条目会被跳过。

Token 数量来自模型服务在 API 响应中返回的 `usage` 字段，并非插件估算。如果服务端没有返回用量数据，进度窗口会明确显示“API 未返回用量统计”。

## 笔记内容

可以在“Zotero 设置 → Zotero AI Note → 笔记内容”中选择需要生成的章节。以下八项默认全部选中：
...
取消勾选后，提示词不会再要求模型输出对应章节。例如取消“关键词”，生成的笔记将只包含其余已选部分。生成前至少需要保留一个选项。

笔记标题下会显示生成人（当前 Zotero 用户）、本地生成时间和实际使用的服务与模型。若 Zotero 未提供用户姓名，则显示账号用户名；仍无法取得时显示“当前用户”。笔记底部会说明 PDF 文本是否被截断。AI 生成内容可能有误，请对照原文核验样本量、效应量、统计结果和结论。
...
### DeepSeek 已响应，但未返回文本内容

请升级到 `0.2.5` 或更高版本。DeepSeek 的推理模式可能先消耗输出 token，导致较短的连接测试没有剩余空间输出正文。新版在 DeepSeek 连接测试和文献总结中关闭推理模式，并在响应仍为空时显示 `finish_reason` 等诊断信息。
...
先在 `CHANGELOG.md` 顶部写明新版本的实际修改，再创建与 `manifest.json` 版本一致的标签，例如 `v0.2.9`。GitHub Actions 会运行检查、构建 XPI，并把该版本的修改说明写入 GitHub Release，而不是只展示自动生成的代码差异。
