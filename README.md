...
插件在首次请求中就会告诉模型输出上限，并要求把正文控制在约上限的 75% 以内，按所选章节平均规划篇幅、优先保留关键信息。`max_tokens` 是 API 的硬上限；提示词只能引导模型，不能保证它精确计算 token 或一定完整输出。如果 API 返回 `length`，或结果缺少所选章节，插件会提示失败，**不会保存残缺笔记，也不会自动重试**。需要时可提高上限后手动重新生成。
